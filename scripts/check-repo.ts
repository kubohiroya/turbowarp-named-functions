...
function checkPolicy() {
  ...
  if (policy.productName !== 'TurboWarp-Named-Functions') {
...
function checkPackageMetadata() {
  ...
  if (packageMetadata.repository?.url !== 'git+https://github.com/kubohiroya/turbowarp-named-functions.git') {
    errors.push('package.json repository.url must point to the current repository');
  }
  if (packageMetadata.bugs?.url !== 'https://github.com/kubohiroya/turbowarp-named-functions/issues') {
...
function checkReadmes() {
  ...
  if (!readmeJa.startsWith('# TurboWarp-Named-Functions\n')) {
...
async function checkPackContents() {
  ...
  if (!files.has('dist/turbowarp-named-functions.js')) {
